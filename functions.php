...
function tmpmela_setup() {
	...
	* replace to change 'expend' to the name of your theme in all
	* template files.
	*/
	load_theme_textdomain( 'expend', get_template_directory() . '/languages' );
	/*
	...
	register_nav_menus( array(
		'primary'   => esc_html__( 'TM Header Navigation', 'expend' ),
		'header-menu'   => esc_html__( 'TM Header Top Links', 'expend' ),
		'topbar-menu'   => esc_html__( 'TM TopBar Links', 'expend' ),
		'footer-menu'   => esc_html__( 'Footer Menu Links', 'expend' ),
	) );
	/*
...
function tmpmela_widgets_init() {
	register_sidebar( array(
		'name' => esc_html__( 'Main Sidebar', 'expend' ),
		'id' => 'sidebar-1',
		'description' => esc_html__( 'Appears on posts and pages except the optional Front Page template, which has its own widgets', 'expend' ),
...
function tmpmela_fonts_url() {
	 ...
	 */
	$source_sans_pro = _x( 'on', 'Source Sans Pro font: on or off', 'expend' );
	/* Translators: If there are characters in your language that are not
	 * supported by Bitter, translate this to 'off'. Do not translate into your
	 * own language.
	 */
	$bitter = _x( 'on', 'Bitter font: on or off', 'expend' );
...
<div class="contributor">
  <div class="contributor-info">
    ...
    <div class="contributor-summary">
      ...
      <a class="contributor-posts-link" href="<?php echo esc_url( get_author_posts_url( $contributor_id ) ); ?>"> <?php printf( _n( '%d Article', '%d Articles', $post_count, 'expend' ), $post_count ); ?> </a> </div>
    <!-- .contributor-summary -->
...
function tmpmela_wp_title( $title, $sep ) {
	...
	if ( $paged >= 2 || $page >= 2 ) {
		$title = "$title $sep " . sprintf( esc_html__( 'Page %s', 'expend' ), max( $paged, $page ) );
	}
	return $title;
}
...
<div class="related_posts_outer"><div class="shortcode-title left"><h1 class="normal-title"><?php echo esc_attr_e( 'Related Posts', 'expend' ) ?></h1></div>
...
return array(
 array(
    'import_file_name'            => 'expend',
    'local_import_file'           => trailingslashit( get_stylesheet_directory() ) . 'demo-content/default/expend.wordpress.xml',
    'local_import_customizer_file'=> trailingslashit( get_stylesheet_directory() ) . 'demo-content/default/expend_customizer_export.dat',
	'local_import_widget_file'    => trailingslashit( get_stylesheet_directory() ) . 'demo-content/default/expend_widgets_settings.wie',
    'import_notice'               => esc_html__( 'Please waiting for a few minutes, do not close the window or refresh the page until the data is imported.', 'expend' ),
),
);
}
...
function sampledata_after_import($selected_import) {
       ...
       if ( class_exists( 'RevSlider' ) ) {
           ...
           echo esc_html__( 'Slider processed', 'expend' );
      }
}
...
add_action( 'wp_ajax_nopriv_update_wishlist_count', 'tmpmela_update_wishlist_count' );
	...
	/*Digital Dwellers additions SEP 2022*/
	function wpb_add_google_fonts() {
		wp_enqueue_style( 'wpb-google-fonts', 'https://fonts.googleapis.com/css2?family=Kanit:ital,wght@0,200;0,400;1,200;1,400&display=swa', false );
	}
	add_action( 'wp_enqueue_scripts', 'wpb_add_google_fonts' );

	function ddwellers_scripts_styles() {		
//		wp_enqueue_script('jquery_3_2_1',get_stylesheet_directory_uri().'/js/ddwellers/jquery-3.2.1.js');
		wp_enqueue_script('jquery_plugins',get_stylesheet_directory_uri().'/js/ddwellers/plugins.js');
		wp_enqueue_script('marquee',get_stylesheet_directory_uri().'/js/ddwellers/jquery.marquee.min.js');
		wp_enqueue_script('vertical_sly',get_stylesheet_directory_uri().'/js/ddwellers/sly.min.js');
		wp_enqueue_script('velocity',get_stylesheet_directory_uri().'/js/ddwellers/velocity.min.js');
		wp_enqueue_script('velocity-min-js',get_stylesheet_directory_uri().'/js/ddwellers/velocity-master/velocity.min.js');
		wp_enqueue_script('velocity-ui',get_stylesheet_directory_uri().'/js/ddwellers/velocity-master/velocity.ui.js');
		wp_enqueue_style('ddwellers_main',get_stylesheet_directory_uri().'/css/main.css');
	}
	add_action( 'wp_enqueue_scripts', 'ddwellers_scripts_styles' );

	function ddwellers_custom_cases_page($content){
		if(is_page('custom-cases')){
//			include 'custom-cases/index.php';
			$content = include dirname(__FILE__).'/custom-cases/index.php';
			return $content;
		}
	}
	add_shortcode('custom-cases','ddwellers_custom_cases_page');
	
	function ddwellers_btm_notice_shipping() {
		echo '<h1 class="allow" style="text-align: center; font-size: 30px; line-height: normal; padding: 40px 0;">For deliveries your shipping fee would be calculated based on location and you will be notified.</h1>';
	}
	add_action( 'woocommerce_review_order_before_submit', 'ddwellers_btm_notice_shipping' ); 
?>
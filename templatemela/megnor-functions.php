...
/*  General Settings  */
	...
	add_option("tmpmela_logo_image_alt",'expend'); // set logo image alt
	...
	add_option("tmpmela_mob_logo_image_alt",'expend'); // set logo image alt		
	...
	add_option("tmpmela_bodyfont_color","2a2a2a"); // body font color
	...
	add_option("tmpmela_button_color","DDDDDD"); // button color
	add_option("tmpmela_button_text_color","000000"); // button Text color	
	add_option("tmpmela_button_hover_color","FED700"); // button hover color
	add_option("tmpmela_button_hover_text_color","000000"); // button hover Text color
	
	/*  Header Settings  */	
	add_option("tmpmela_header_background_upload", get_stylesheet_directory_uri() . "/images/megnor/bg.jpg"); // Header Background image
	...
	add_option("tmpmela_header_bottom_bkg_after_color","3b3b3b"); // header bottom background color
	
	add_option("tmpmela_show_header_contact","yes"); // Show Header Right Service CMS
	add_option("tmpmela_header_contact_text","Support"); // Header Right Service Text 
	add_option("tmpmela_header_contact_number","1800 5555 3636"); // Header Right Service Text Color
	
	add_option("tmpmela_top_menu_text_color","2a2a2a"); // Top Menu Text color
	add_option("tmpmela_top_menu_texthover_color","0062bd"); // Top Menu Text hover color
	...
	add_option("tmpmela_sub_menu_texthover_color","0062bd"); // Sub Menu Text hover color
    /*  Navigation Menu Setting  */
	add_option("tmpmela_navbar_category_title","All Categories"); // Sidebar Category Title
	add_option("tmpmela_navbar_category_title_bkg_color","2a2a2a"); // Sidebar Category Title Bkg Color
	add_option("tmpmela_category_title_text_color","FFFFFF"); // Header Category Title text color
	add_option("tmpmela_sidebar_category_bg_color","FFFFFF"); // Sidebar Category Background Color
	add_option("tmpmela_sidebar_category_link_color","2a2a2a"); // Sidebar Category Link Color
	add_option("tmpmela_sidebar_category_link_hover_color","0062bd"); // Sidebar Category link Hover Color
	add_option("tmpmela_sidebar_category_child_link_color","2a2a2a"); // Sidebar Category Child Link Color
	add_option("tmpmela_sidebar_category_child_link_hover_color","0062bd"); // Sidebar Category Child link Hover Color
	add_option("tmpmela_sidebar_category_sub_child_link_color","2a2a2a"); // Sidebar Category Sub Child Link Color
	add_option("tmpmela_sidebar_category_sub_child_link_hover_color","0062bd"); // Sidebar Category Sub Child link Hover Color
	
	/*  Top Bar Banner Settings  */	
	add_option("tmpmela_show_topbar","yes"); // show top bar
	add_option("tmpmela_topbar_color","FFFFFF"); // topbar color1
	add_option("tmpmela_topbar_bkg_opacity","0.3"); // TopBar background opacity
	add_option("tmpmela_topbar_text_color","666666"); // TopBar Text color	
	add_option("tmpmela_topbar_link_color","666666"); // topbar_link_color
	add_option("tmpmela_topbar_link_hover_color","0062bd"); // top bar_link_hover_color
	add_option("tmpmela_show_offer","yes"); // show top bar
	add_option("tmpmela_show_offer_text",'Welcome To Worldwide expend Store'); // offer text
	...
	add_option("tmpmela_hoverlink_color","0062bd"); // link hover color
	
	/*  Sidebar setting for other pages  */
	add_option("tmpmela_page_sidebar","yes");
	
	/*  Footer Settings  */	
	add_option("tmpmela_footer_newsletter_bkg_color","2a2a2a"); // Footer Background color
	add_option("tmpmela_footer_bkg_color","f2f2f2"); // Footer Background color
	add_option("tmpmela_footer_bottom_bkg_color","FFFFFF"); // Footer Bottom Background color
	add_option("tmpmela_footer_bottom_bkg_color_opacity","0.3"); // Footer Bottom Background color Opacity
	add_option("tmpmela_footer_background_upload", get_stylesheet_directory_uri() . "/images/megnor/bg.jpg");//Footer Background image
	add_option("tmpmela_footer_back_repeat","repeat");//Background repeat
	add_option("tmpmela_footer_back_position","center+center");//Background position
	add_option("tmpmela_footer_back_attachment","scroll"); //Background attachment
	add_option("tmpmela_footer_title_color","2a2a2a"); // Footer link text color
	add_option("tmpmela_footerlink_color","666666"); // Footer link text color
	add_option("tmpmela_footerhoverlink_color","0062bd"); // Footer link hover text color
...
function tmpmela_header_contact() {
    echo '<div class="contact-block">	
	<div class="contact-icon"></div>
	<div class="contact-block-inner">	
	<div class="service-text text1">'.get_option('tmpmela_header_contact_text').'</div>
	<div  class="service-text text2">'.get_option('tmpmela_header_contact_number').'</div>
	</div>
	</div>';
}
...
function tmpmela_excerpt($limit) 
{
      ...
      if (count($excerpt)>=$limit) {
        array_pop($excerpt);
        $excerpt = implode(" ",$excerpt).'.<div class="read-more"><a class="read-more-link" href="'.esc_url(get_permalink()).'">'.esc_html__( 'Read More' , 'expend' ) .'</a></div>';
      } else {
...
function tmpmela_load_fonts() {
	...
	$Roboto = _x( 'on', 'Roboto font: on or off', 'expend' ); 
	$OpenSans = _x( 'on', 'Open Sans Font: on or off', 'expend' );
    if (  'off' !== $Roboto || 'off' !== $OpenSans ) {
			$font_families = array(); 
			if ( 'off' !== $Roboto ) {
...
function tmpmela_search_form( $form ) {
    ...
    <div><label class="screen-reader-text" for="s">' . esc_attr_x( 'Search for:','label','expend' ) . '</label>
    <input class="search-field" type="text" placeholder="'. esc_attr__( 'Search', 'expend' ) .'" value="' . get_search_query() . '" name="s" id="s" />
    <input class="search-submit" type="submit" id="searchsubmit" value="'. esc_attr_x( 'Go','label','expend' ) .'" />
    </div>
    </form>';
    return $form;
}
...
<li <?php comment_class(); ?> id="comment-<?php comment_ID(); ?>">
  <p>
    <?php esc_html_e( 'Pingback:', 'expend' ); ?>
    <?php comment_author_link(); ?>
    <?php edit_comment_link( esc_html__( '(Edit)', 'expend' ), '<span class="edit-link">', '</span>' ); ?>
  </p>
</li>
<?php
			break;
...
<li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
  <div id="comment-<?php comment_ID(); ?>" class="comment-body">
    ...
    <div class="author-content">
      ...
      <?php edit_comment_link( esc_html__( 'Edit', 'expend' ), ' ' ); ?>
      <div class="clearfix"></div>
      <abbr class="published" title="<?php echo get_the_title() ?>"><?php printf( esc_html__( '%1$s at %2$s', 'expend' ), get_comment_date(),  get_comment_time() ); ?></abbr> </div>
    ...
    <?php esc_html_e( 'Your comment is awaiting moderation.', 'expend' ); ?>
    </em> <br />
    <?php endif; ?>
  </div>
  <!-- #comment-##  -->
...
function tmpmela_sticky_post() {
	if ( is_sticky() && is_home() && ! is_paged() )
		echo '<div class="meta-inner"><span class="sticky-post">' . esc_html__( 'Sticky', 'expend' ) . '</span></div>';
}
endif;
if ( ! function_exists( 'tmpmela_categories_links' ) ) :
function tmpmela_categories_links() {
	// Translators: used between list items, there is a space after the comma.
	$categories_list = get_the_category_list( esc_html__( ', ', 'expend' ) );
	if ( $categories_list ) {
...
function tmpmela_tags_links() {
	// Translators: used between list items, there is a space after the comma.
	$tag_list = get_the_tag_list( '', esc_html__( ', ', 'expend' ) );
	if ( $tag_list ) {
...
function tmpmela_author_link() {
	...
	if ( 'post' == get_post_type() ) {
		printf( '<div class="meta-inner"><span class="author vcard"><i class="fa fa-pencil-square-o"></i><a class="url fn n" href="%1$s" title="%2$s" rel="author">'.esc_html__("by ",'expend').'%3$s</a></span></div>',
			esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ),
			esc_attr( sprintf( esc_html__( 'View all posts by %s', 'expend' ), get_the_author() ) ),
			get_the_author()
		);
	}
}
...
function tmpmela_comments_link() {
	...
	comments_popup_link( esc_html__( 'Leave a Comment', 'expend' ), esc_html__( 'Comment (1)', 'expend' ), esc_html__( 'Comments (%)', 'expend' ) );
	echo '</span></div>';
	endif; 
}
endif;
...
function tmpmela_entry_date( $echo = true ) {
	if ( has_post_format( array( 'chat', 'status' ) ) )
		$format_prefix = _x( '%1$s on %2$s', '1: post format name. 2: date', 'expend' );
	else
		$format_prefix = '%2$s';
...
<time class="entry-date" datetime="%3$s">%4$s</time></a></span></div>',
		...
		esc_attr( sprintf( esc_html__( 'Permalink to %s', 'expend' ), the_title_attribute( 'echo=0' ) ) ),
...
function tmpmela_shortcode_paging_nav() {
	...
	$links = paginate_links( array(
		...
		'prev_text' =>  wp_kses( __( '<i class="fa fa-angle-left"></i>', 'expend' ),tmpmela_allowed_html()),
		'next_text' =>  wp_kses( __( '<i class="fa fa-angle-right"></i>', 'expend' ),tmpmela_allowed_html()),
	) );
	if ( $links ) :
...
add_image_size( 'tmpmela-blog-posts-list', 1400, 600, true );
...
function tmpmela_out_of_stock() {
 global $product;
 	if ( !$product->is_in_stock() ) {
        echo '<div class="soldout_wrapper"><span class="soldout">'.esc_html__('SOLD OUT','expend').'</span></div>';
    }
}
endif;
...
if ( in_array( 'woocommerce/woocommerce.php' ,apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) )  && in_array( 'TemplatemelaCustomPost/TemplatemelaCustomPost.php' , apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) )  {
add_action( 'woocommerce_share', 'tmpmela_brand' );
function tmpmela_brand() {
	...
	if(!empty($term_list) ):?>
		<div class="brand">
			<?php  echo wc_attribute_label(  __('Brand','expend') ).':'; ?>
...
if( ! function_exists( 'tmpmela_add_to_wishlist_in_menu' ) ){
    function tmpmela_add_to_wishlist_in_menu(){
		...
		echo esc_html_e('Wishlist','expend');
		echo '</span>';
...
?>
        <span class="counter-label"><?php esc_html_e('Compare','expend'); ?></span>
        <span class="compare-count"><?php count('[yith_woocompare_counter]'); ?></span>
<?php
        echo'</a>';
    }
}
...
{
  	if ( $rating > 0 ) {
    	$title = sprintf( __( 'Rated %s out of 5', 'expend' ), $rating );
  	} else {
    	$title = 'Not yet rated';
    	$rating = 0;
  	}
  	$rating_html  = '<div class="star-rating" title="' . $title . '">';
    $rating_html .= '<span style="width:' . (($rating / 5) * 100) . '%"><strong class="rating">' . $rating . '</strong> ' . esc_attr__('out of 5', 'expend') . '</span>';
  	$rating_html .= '</div>';
  	return $rating_html;
	}
...
$label     = $req ? '*' : ' ' . __( '(optional)', 'expend' );
$aria_req  = $req ? "aria-required='true'" : '';
$fields['author'] =
'<p class="comment-form-author comment-block">
<input id="author" name="author" type="text" placeholder="' . esc_attr__( "Name*", "expend" ) . '" value="' . esc_attr( $commenter['comment_author'] ) .
'" size="30" ' . $aria_req . ' />
</p>';
$fields['email'] =
'<p class="comment-form-email comment-block">
<input id="email" name="email" type="email" placeholder="' . esc_attr__( "E-mail*", "expend" ) . '" value="' . esc_attr( $commenter['comment_author_email'] ) .
'" size="30" ' . $aria_req . ' />
</p>';
$fields['url'] =
'<p class="comment-form-url comment-block">
<input id="url" name="url" type="url"  placeholder="' . esc_attr__( "Website", "expend" ) . '" value="' . esc_attr( $commenter['comment_author_url'] ) .
'" size="30" />
</p>';
return $fields;
}
...
//product Countdown
/*function tmpmela_product_countdown() {
	global $product; 
	
	$today = date('Y-m-d');
	$sale_price_dates_from = $product->get_date_on_sale_from();
	$sale_price_dates_to = $product->get_date_on_sale_to();
	$sale_price_dates_from = substr($sale_price_dates_from, 0, -15);
	$sale_price_dates_to = substr($sale_price_dates_to, 0, -15);
	if ($today >= $sale_price_dates_from  && $today <= $sale_price_dates_to){
		if ($sale_price_dates_to != ""){
			echo '<div class="count-down product-count-down">';	
			echo '<div class="countbox hastime" data-time="'.$sale_price_dates_to.'">';	
			echo '</div></div>';	
		}
	}
}
add_action( 'woocommerce_after_shop_loop_item', 'tmpmela_product_countdown', 10); */
?>
...
<style type="text/css">
	...
	.header-bottom::after{
		background-color:#<?php echo (esc_attr(get_option('tmpmela_header_bottom_bkg_after_color'))); ?>;
	}
	.box-category, .header-cart{
		background-color:#<?php echo (esc_attr(get_option('tmpmela_navbar_category_title_bkg_color'))); ?>;
	}
	 .box-category .title1,
	 .header-cart .cart-contents .cart-label,.header-cart .cart-contents span.cart-qty, .whislist-counter .count
	 {color:#<?php echo esc_attr(get_option('tmpmela_category_title_text_color')); ?>;}
